...
func (r *RefreshTokenRepository) CreateRefreshToken(ctx context.Context, userID int64, tokenHash, platform, deviceName, userAgent, ipAddress string, expiresAt time.Time) error {
	_, err := r.db.ExecContext(ctx, `INSERT INTO refresh_tokens (user_id, token_hash, platform, device_name, user_agent, ip_address, expires_at) VALUES ($1,$2,NULLIF($3,''),NULLIF($4,''),NULLIF($5,''),NULLIF($6,'')::inet,$7)`, userID, tokenHash, platform, deviceName, userAgent, ipAddress, expiresAt)
	return err
}
func (r *RefreshTokenRepository) GetRefreshTokenByHash(ctx context.Context, tokenHash string) (*RefreshToken, error) {
	q := `SELECT rt.id, rt.user_id, rt.token_hash, rt.platform, rt.device_name, rt.user_agent, host(rt.ip_address), rt.expires_at, rt.revoked_at, rt.last_used_at, rt.created_at, u.email, u.role FROM refresh_tokens rt JOIN users u ON u.id = rt.user_id WHERE rt.token_hash=$1`
	var rt RefreshToken
	err := r.db.QueryRowContext(ctx, q, tokenHash).Scan(&rt.ID, &rt.UserID, &rt.TokenHash, &rt.Platform, &rt.DeviceName, &rt.UserAgent, &rt.IPAddress, &rt.ExpiresAt, &rt.RevokedAt, &rt.LastUsedAt, &rt.CreatedAt, &rt.UserEmail, &rt.UserRole)
	if err != nil {
